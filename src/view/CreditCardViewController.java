...
public class CreditCardViewController implements Initializable {
    ...
    @FXML
    private BarChart<String, Number> barChartCard;
        ...
        for (Card card : list) {
            ...
            if (uniqueYears.add(year)) {
                ...
                for (Card c : obsList) {
                    ...
                    if (year.equals(sdf.format(c.getDate()))) {
                        
                        sum += c.getValue();          
                    }
                }

                xy.getData().add(new XYChart.Data<>(year, sum));
                sum = 0;
                comboBoxDate.getItems().add(year);    
            }
        }
        barChartCard.getData().add(xy);
    }
